...
export const userGetBySteamId = async (steam_id) => {
    let info = await dbGet('SELECT id,name,vk_id,avatar,reg_date,last_login FROM users WHERE steam_id=?', [steam_id])
...
export const getUsers = async (steam_ids) => {
    let ids = steam_ids.split(',').map(x => String(parseInt(x))).join(',')
    console.log(ids)
    let res = await dbGet('SELECT name, id, avatar, last_login FROM users WHERE steam_id IN (' + ids + ')')
    return res;
}